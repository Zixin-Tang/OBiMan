...
class TaskSimulator:
    ...
    def _extract_hand_constraints(self, task_instruction):
        ...
        # regular expression pattern for extracting Workspace constraints
...
class TaskPlannerRule(TaskSimulator):
    ...
    async def run_with_validation(self, task_instruction, task_final_status, image_path):
        ...
        f2 = open(os.path.join(Grounded_Root, "objects.json"), 'w')
        f2.write(b)
        f2.close()
        b = json.dumps(self.hands)
        f2 = open(os.path.join(Grounded_Root, "hands.json"), 'w')
        f2.write(b)
        f2.close()
        b = json.dumps(self.objective)
        f2 = open(os.path.join(Grounded_Root, "objective.json"), 'w')
        f2.write(b)
        f2.close()
        ...
        if check_pass == False:
            ...
            with open(os.path.join(Save_Root, "fail.txt"),'w') as f:   
                f.write("Grounding error.")   

            return False, ["Grounding error."]       
        

        # Generate sequential stages based on task planning module
...
def run_with_TaskPlannerRule(task_instruction, task_final_status, image_path):
    ...
    if check_input:
        ...
        print("---------------------------------------------")
        print(os.path.join(OBiMan_Bench_Root, Task_Name))
    ...
    with open(os.path.join(Root, level), "r", encoding="utf-8") as file:
        for line in file:
            ...
            if len(parts) == 4:
                ...
                Save_Root = os.path.join(OBiMan_Bench_Root, Task_Name, "grounded")
                if not os.path.exists(Grounded_Root):
                    os.mkdir(Grounded_Root)
                else:
                    shutil.rmtree(Grounded_Root)
                    os.mkdir(Grounded_Root)
