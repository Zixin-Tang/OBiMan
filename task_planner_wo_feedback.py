...
class TaskSimulator:
    def __init__(self, agent_type, model_name, temperature):
        ...
        self.multi_round_dialog_times = 5
...
class TaskPlannerRule(TaskSimulator):
    ...
    async def run_with_validation(self, task_instruction, task_final_status, image_path):
        ...
        # For fair comparision, we reuse the generated scenario grounding information from OBiMan-Planner
        if not Load_Grounded_Info:
            ...
            if check_pass == False:
                ...
                with open(os.path.join(Save_Root, "fail.txt"),'w') as f:    #设置文件对象
                    f.write("Grounding error.")               #将字符串写入文件中
        ...
        # Generate sequential stages based on task planning module (w/o image)
    ...
    def convert_stages_to_task_plan(self, stages):
        pass
    ...
    with open(os.path.join(Root, level), "r", encoding="utf-8") as file:
        for line in file:
            ...
            if len(parts) == 4:
                ...
                Grounded_Root = os.path.join(OBiMan_Bench_Root, Task_Name, "grounded")
                Save_Root = os.path.join(OBiMan_Bench_Root, Task_Name, "grounded_wo_feedback")
                ...
                if os.path.exists(os.path.join(OBiMan_Bench_Root, Task_Name, "grounded_wo_feadback")):
                    shutil.rmtree(os.path.join(OBiMan_Bench_Root, Task_Name, "grounded_wo_feadback"))
