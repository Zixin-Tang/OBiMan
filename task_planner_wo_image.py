...
class TaskPlannerRule(TaskSimulator):
    def __init__(self, agent_type, model_name, temperature):
        ...
        self.stage_generation_system_prompt = read_prompt_from_file(file_path="prompts/stage_generation_system_wo_image.txt")
    ...
    async def run_with_validation(self, task_instruction, task_final_status, image_path):
        ...
        for diag_ind in range(self.multi_round_dialog_times):
            ...
            if len(self.stage_list) > Task_GT_Step*2:
                break
...
def run_with_TaskPlannerRule(task_instruction, task_final_status, image_path):
    ...
    if check_input:
        agent_type = "openai"
        model_name = "gpt-4o"
        tp = TaskPlannerRule(agent_type, model_name, temperature=0.27)
    ...
    with open(os.path.join(Root, level), "r", encoding="utf-8") as file:
        for line in file:
            ...
            if len(parts) == 4:
                ...
                Save_Root = os.path.join(OBiMan_Bench_Root, Task_Name, "grounded_wo_image")
